...
let buyFruit = () => {
    ...
    while (wallet >= 0) {
        ...
        if (wallet < .48) {
            let formattedMoney = Number(wallet).toLocaleString('en-US', { style: 'currency', currency: 'USD' });
...
let arrayNormal = () => {
    const myArray = ["HTML", "CSS", "JavaScript", "Python", "Ruby"];
...
let arrayMixed = () => {
    let myArray = [1, true, "ok"];
    for (let i = 0; i < myArray.length; i++) {
        console.log(myArray[i]);
    }
};
let arrayTwoDemensional = () => {
...
let myArray = [1, true, "someValue", [me]];

let newArray = [[1, 2], [3, [me], 4]];
...
let answer1 = (((3 * 90) === 270) || !(false && (!false)) || "bex".toUpperCase() === "BEX");
...
let createPerson = () => {
    const geogre = new Person("George Washington", 275);
    const tim = new Person("Timothy Ingledue", 45);
    console.log(geogre);
    console.log(tim);
};
